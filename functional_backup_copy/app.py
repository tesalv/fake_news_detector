    ...
    if request.method == "POST":
        ...
        if lang=='eng':
            ...
            if predict>=0.5:
                ...
                return render_template("prediction.html", prediction_text=f"Prediction|| Fake News with a confidence level of  {round(confidence,0)} %" , prediction_text1=f"{get_topic(news_to_predict)}", prediction_text2=f"{get_sentiment(news_to_predict)}")
            else:
                confidence=(50-predict[0][0]*100)*2
                return render_template("prediction.html", prediction_text=f"Prediction|| True News with a confidence level of {round(confidence,0)} %",prediction_text1=f"{get_topic(news_to_predict)}", prediction_text2=f"{get_sentiment(news_to_predict)}")
        elif lang=='por':
            print(lang)
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if predict>=0.5:
                confidence=(predict[0][0]*100-50)*2
                return render_template("image.html", prediction_text=f"Prediction|| Fake News with a confidence level of  {round(confidence,0)} %" , prediction_text1=f"{get_topic(news_to_predict)}", prediction_text2=f"{get_sentiment(news_to_predict)}")
            else:
                confidence=(50-predict[0][0]*100)*2
                return render_template("image.html", prediction_text=f"Prediction|| True News with a confidence level of {round(confidence,0)} %",prediction_text1=f"{get_topic(news_to_predict)}", prediction_text2=f"{get_sentiment(news_to_predict)}")
